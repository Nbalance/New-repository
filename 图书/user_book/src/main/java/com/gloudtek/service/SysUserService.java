...
public interface SysUserService {
	...
	//查询用户账号是否重复
	SysUserEntity checkemail(String mail);
	//获取邮箱验证修改密码
	int updatepass(SysUserEntity user);
	//获取用户在线用户总数
	int lineTotal();
	//获取离线用户总数
	int linesTotal();
	//修改用户在线状态
	int updateline(SysUserEntity user);
	//修改用户头像url
	int updateurl(SysUserEntity user);
}
