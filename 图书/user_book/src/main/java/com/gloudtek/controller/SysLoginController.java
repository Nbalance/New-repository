...
import com.gloudtek.utils.Session.Cookies;
import com.gloudtek.utils.Session.Session;
...
public class SysLoginController extends AbstractController {
	...
	public R login(HttpServletResponse response, HttpServletRequest request, @RequestBody LoginParam loginParam,
			LogInfo logInfo, Logutil logutil) throws IOException {
		...
		cs.sessionis(response, request, loginParam);
		//相同账号只能在一处登录
		Session ss =new Session();
		ss.sessionis(response, request, loginParam);
		SysUserEntity sys =new SysUserEntity();
		sys.setUsername(loginParam.getUsername());
		sys.setLine((long) 1);
		sysUserService.updateline(sys);
		SysLogEntity syslog = new SysLogEntity();
		syslog.setUsername(loginParam.getUsername());
		syslog.setType(logInfo.LOG_TYPE_LOGIN);
		syslog.setDetail(logInfo.LOG_DETAIL_USER_LOGIN);
	...
	public R logout(HttpServletRequest request,@RequestBody LoginParam loginParam,LogInfo logInfo,Logutil logutil) {
		SysUserEntity sys =new SysUserEntity();
		sys.setUsername(loginParam.getUsername());
		sys.setLine((long) 0);
		sysUserService.updateline(sys);
		SysLogEntity syslog = new SysLogEntity();
		syslog.setUsername(loginParam.getUsername());
		syslog.setType(logInfo.LOG_TYPE_LOGOUT);
		syslog.setDetail(logInfo.LOG_DETAIL_USER_LOGOUT);
		syslog.setIp(Logutil.getClientIp(request));
		syslog.setPlace(Logutil.getAddressByIP(request));
		sysLogService.save(syslog);
		return R.ok();
	...
	public R password(HttpServletRequest request,Long userId,String password, String newPassword,LogInfo logInfo) {
		...
		syslog.setDetail(logInfo.LOG_DETAIL_UPDATE_PASSWORD);
		syslog.setIp(Logutil.getClientIp(request));
		syslog.setPlace(Logutil.getAddressByIP(request));
		sysLogService.save(syslog);
		return R.ok();
	...
	// 重置登录用户密码（邮箱更改）
	@RequestMapping("/sys/user/resetPassword")
	public R resetPassword(@RequestBody LoginParam loginParam) throws Exception {
		SysUserEntity sysUserEntity = sysUserService.queryByUserName(loginParam.getUsername());
		...
		String sid = new String(Base64.getEncoder()
				.encode((loginParam.getUsername() + "," + password + "," + sendTime).getBytes("utf-8")));
	...
	@ResponseBody
	public R sessionOut() {
		return R.error(401, "您的账号在其他地方登陆,您被迫下线");
	}
}
